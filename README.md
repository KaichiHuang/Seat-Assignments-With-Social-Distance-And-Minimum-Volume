...
以165個可用座位的演講廳為例，在社交距離為20英吋(約50公分)、音量限制為在座位上至少需聽到45分貝的聲音，並將老師位置設定為[100,125,0]，可以得出最佳解為ＸＸ個座位，如下圖所示。

##### 3.3.1.1 考慮設置一個擴音設備

從上圖我們可以發現左邊區域被選取的狀況較少，因此加以考慮新增一個擴音設備作為情境，而所得的結果為在五個候選擴音設備位置：A=[0,15,70]、B= [0,35,70]、
C= [0,55,70]、D= [0,75,70]、E= [0,95,70]上，均可以獲得最佳解：35個座位，以下圖為選取X擴音設備[]作為示範。
